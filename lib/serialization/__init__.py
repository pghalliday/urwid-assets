...
from typing import Any, get_type_hints, Callable, Type, TypeVar
...
def _deserialize(arg: Any, field_type: type):
    type_arg: type | None = None
    if hasattr(field_type, '__origin__') and hasattr(field_type, '__args__'):
        type_arg = field_type.__args__[0]
        field_type = field_type.__origin__
    if issubclass(field_type, tuple):
        assert isinstance(arg, list)
        return tuple(_deserialize(item, type_arg) for item in arg)
