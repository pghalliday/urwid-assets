...
class Table(WidgetWrap, Generic[DATA]):
    ...
    def with_current_row_data(self, callback: Callable[[DATA], None]) -> Callable[[], None]:
        def _with_current_row_data() -> None:
            row = self.get_focused()
            if row is not None:
                callback(row.data)

        return _with_current_row_data
